"""
Implementation for EdgeLLM architecture
"""
...
class EdgeLLMConfig(ConfigBase):  # pylint: disable=too-many-instance-attributes
    """Configuration of the EdgeLLM model."""
    ...
    def __post_init__(self):
        ...
        assert self.num_key_value_heads  % self.num_attention_heads == 0
...
class EdgeLLMMLP(nn.Module):
    def __init__(self, config: EdgeLLMConfig, hidden_size=None, intermediate_size=None):
        super().__init__()
...
class EdgeLLMRotaryEmbedding(nn.Module):
    def __init__(self, config: EdgeLLMConfig):
        self.rope_fn = position_embedding.rope_freq_default
    ...
    def forward(
        ...
        def _rope(x: te.Tensor, positions: te.Tensor):
            ...
            return te.compute(x.shape, compute, name="rope")
...
class EdgeLLMEmbedding(nn.Embedding):
...
class EdgeLLMAttention(nn.Module):  # pylint: disable=too-many-instance-attributes
    def __init__(self, config: EdgeLLMConfig):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = EdgeLLMRotaryEmbedding(config)
...
class EdgeLLMDecoderLayer(nn.Module):
    def __init__(self, config: EdgeLLMConfig, layer_idx: int):
        super().__init__()
        self.self_attn = EdgeLLMAttention(config)
        ...
        def _set_tp():
            ...
            assert isinstance(self.mlp, EdgeLLMMLP)
            _set(
                self.mlp.up_proj.weight,
                tp.ShardSingleDim("_shard_up", dim=0),
            )
            _set(
...
class EdgeLLMModel(nn.Module):
    def __init__(self, config:EdgeLLMConfig):
        self.embed_tokens = EdgeLLMEmbedding(config.vocab_size, config.hidden_size)
        self.layers = nn.ModuleList(
            [
                EdgeLLMDecoderLayer(config, layer_idx)
...
class EdgeLLMForCausalLM(nn.Module):  # pylint: disable=too-many-instance-attributes
    def __init__(self, config: EdgeLLMConfig):
